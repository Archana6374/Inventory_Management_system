# Inventory_Management_system
This is a project which shows a basic inventory managements where it allows the supplier and the warehouse to run with a little less manual intervention.

#To execute follow the below steps 
1.download the files in the repository under the folder inventory_management_system
2.open terminal 
3.execute the commmand :
   javac main.java 
or
run the main.java file by clicking run in the top tool bar in your IDE.

The approach used here is done by separating the modules into well defined classes(entites) that allows the project to include abstraction,encapsulation and ingeritance in the project.
